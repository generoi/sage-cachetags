...
class Polylang implements Action
{
    ...
    public function filterArchiveTags(array $tags): array
    {
        ...
        if (! $lang) {
            error_log('[Polylang CacheTags] filterArchiveTags: no language context, passing through: '.implode(', ', $tags));

            return $tags;
        }

        $result = array_map(function (string $tag) use ($lang) {
            $parts = explode(':', $tag);

            if (count($parts) === 2 && $parts[0] === 'archive' && pll_is_translated_post_type($parts[1])) {
                error_log("[Polylang CacheTags] filterArchiveTags: {$tag} -> {$tag}:{$lang}");
    ...
    public function onPostStatusTransition(string $newStatus, string $oldStatus, WP_Post $post): void
    {
        ...
        if ($lang) {
            error_log("[Polylang CacheTags] onPostStatusTransition: {$post->post_type} #{$post->ID} ({$newStatus} <- {$oldStatus}) -> archive:{$post->post_type}:{$lang}");
